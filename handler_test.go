...
func TestCreateHandlersForConfig(t *testing.T) {
	...
	t.Run("should successfully GET 3x a target file first with target-internal user, but never call casHandler", func(t *testing.T) {
		...
		requestUrl := prepareServiceUnderTest(t, nexusHandler, casHandler)
...
func prepareServiceUnderTest(t *testing.T, nexusHandler http.HandlerFunc, casHandler http.HandlerFunc) *url.URL {
	t.Helper()
	...
	sut, err := createHandlersForConfig(conf)
	require.NoError(t, err)
